...
- upload diretto ChatGPT live: `upload_image_file` con `_meta["openai/fileParams"]`, `file_id` opaco, receiver del `fetch` Cloudflare preservato, HTTPS/redirect/timeout/size controllati, firma e dimensioni reali, `FixedLengthStream` verso R2 quando la misura e' dichiarata e fallback bounded-buffer quando manca;
